...
def anime_details(anime_id):
    ...
    search_query = request.args.get('q' , "").strip() #to get the searched keyword.

    anime = JikanService.get_anime_details(anime_id)

    #if anime not found, show error
    if not anime:
        flash(f'Anime with {search_query} not found', 'danger')
        return redirect(url_for('main.index'))
    return render_template('main/anime_details.html', anime=anime)